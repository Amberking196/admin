 ...
 */ 
@Repository
public class  RepairRecordVmCodeDaoImpl extends BaseDao<RepairRecordVmCodeBean> implements RepairRecordVmCodeDao{

private static Log log = LogFactory.getLog(RepairRecordVmCodeDaoImpl.class);
public ReturnDataUtil listPage(RepairRecordVmCodeCondition condition) {
ReturnDataUtil data=new ReturnDataUtil();
StringBuilder sql=new StringBuilder();
sql.append("select id,vmCode,createTime,updateTime,createUser,reason,plan,remark,state from repair_record_vmCode where 1=1 ");
...
pst.setObject(i+1, plist.get(i));}
rs = pst.executeQuery();
List<RepairRecordVmCodeBean> list=Lists.newArrayList();
while(rs.next()){
RepairRecordVmCodeBean bean = new RepairRecordVmCodeBean();
...
RepairRecordVmCodeBean entity=new RepairRecordVmCodeBean();
