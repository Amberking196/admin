...
public class TblCustomerComplainReplyDaoImpl extends BaseDao<TblCustomerComplainReplyBean> implements TblCustomerComplainReplyDao {
    ...
    public List<TblCustomerComplainReplyBean> list(Integer complainId,Integer pid) {
        ...
        sql.append("select id,complainId,pid,src,content,createTime,createUser,createName from tbl_customer_complain_reply where 1=1 ");
        if(complainId!=null){
            sql.append(" and complainId="+complainId);
        }
        if (pid!=null){
    ...
    public List<TblCustomerComplainReplyBean> listReplyBean(Long complainId) {
        log.info("<TblCustomerComplainReplyDaoImpl>-------<listReplyBean>------start");
        StringBuilder sql = new StringBuilder();
        ...
        try {
            ...
            log.info("<TblCustomerComplainReplyDaoImpl>-------<listReplyBean>------end");
            return list;
        } catch (SQLException e) {
            e.printStackTrace();
            log.error(e.getMessage());
