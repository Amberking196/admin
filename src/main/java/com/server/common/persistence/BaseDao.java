...
public class BaseDao<E> extends MySqlFuns{
  ...
  public Object insert(Connection conn,Object entity){
	...
	public boolean update(Connection conn,E entity){
		Row row = paserEntity(entity);
		String sql=row.getUpdateSql();
		return execute(conn,row, sql);
	}

    public boolean del(E entity){
		
		Row row = paserEntityWithOutNullField(entity);
	 ...
	 * @throws SQLException
	 */
	private boolean execute(Connection conn,Row row, String sql) {
		PreparedStatement pst=null;
		try {
			pst=conn.prepareStatement(sql);
			List<Object> parameterList=Lists.newArrayList();
				...
				try {
					...
					if (name.indexOf("get")==0 && !name.equals("getClass")){
					   ...
					   NotField nf=field.getAnnotation(NotField.class);
						...
						NullField nullfield=field.getAnnotation(NullField.class);
						if(temp!=null || nullfield!=null){
	 ...
	 * @return
	 */
	private Row paserEntityWithOutNullField(E entity) {
		Row row=new Row();
		Entity ann=(Entity)entity.getClass().getAnnotation(Entity.class);
		if (ann!=null){
			row.setTableName(ann.tableName());
	...
	private Object cloumnsToEntity(ResultSet rs1, Class class1) {
		...
		for(int i=1;i<=ccount;i++){
			String fieldName=md.getColumnName(i);
		     ...
		     //给id 属性赋值
			Object value=rs1.getObject(fieldName);
            if(value==null){
            	continue;
			}
			if(!field.getType().equals(value.getClass())){
				if(field.getType().toString().equals("class java.lang.Long")){
					if(value.getClass().toString().equals("class java.lang.Integer")){
						value=Long.parseLong(((Integer)value).toString());
					}
				}
...
//		            		String fieldName=md.getColumnName(i);
  ...
  public void updateSql(String sql,List<Object> parameterList){
	...
	//分组查询统计总数
	public String countGroupSql(String sql) {
		int start=sql.indexOf("from");
		int end=sql.indexOf(" order ", start);
		if (end==-1)
		  sql="select count(*) from (select 1 "+sql.substring(start)+")A";
		else
		   sql="select count(*) from (select 1 "+sql.substring(start,end)+")A";
		return sql;
	}
	 /**
     * 多条数据封装到实体类列表
