...
public class AppLoginInfoDaoImpl extends MySqlFuns implements AppLoginInfoDao{

	public static Logger log = LogManager.getLogger(AppLoginInfoDaoImpl.class); 
	...
	public LoginInfoBean findLoginInfo(String alipayUserId) {
		...
		sql.append(" FROM login_info where aliUserId = '"+alipayUserId+"'");
	...
	public LoginInfoBean queryByPhone(String phone) {
		...
		sql.append(" rightsCompany,rightsMachines,rightsLine,rightsButton,validataCode,");
		sql.append(" outDate,banVmCode,openId,aliUserId,isPrincipal,post,departMent");
		sql.append(" FROM login_info where phone = '"+phone+"'");
		log.info("sql语句："+sql);
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public LoginInfoBean queryById(Long id) {
		...
		sql.append(" companyId,phone,loginModel,mail,founder,updateDate,extraMenu,");
		sql.append(" rightsCompany,rightsMachines,rightsLine,rightsButton,validataCode,");
		sql.append(" outDate,banVmCode,openId,aliUserId,isPrincipal,post,departMent,isExpatriate,level,areaId,lineId ");
		sql.append(" FROM login_info where id = "+id);
		log.info("sql语句："+sql);
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {
			...
			while(rs!=null && rs.next()){
				...
				loginInfo.setRole(rs.getString("role"));
				loginInfo.setStatus(rs.getInt("status"));
				loginInfo.setLevel(rs.getInt("level"));
				loginInfo.setAreaId(rs.getInt("areaId"));
				loginInfo.setLineId(rs.getInt("lineId"));
				loginInfo.setIsExpatriate(rs.getInt("isExpatriate"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
