...
public class WxTicketService {
	...
	public String getTicket(MyWXGzhConfig myWXGzhConfig) {
		...
		try {
			...
			String access_token = "";
			String ticket = "";
			String apiticket = redisClient.get(redisJsapiTicket);
			log.info("[apiticket] =" + apiticket);
			if (StringUtil.isBlank(apiticket)) {
				Lock secondLock = accessTokenLock;
				try{
					secondLock.lock();
					String act = redisClient.get(redisAccessToken);
					log.info("[act] ="+act);
					if (StringUtil.isBlank(act)) {
						...
						Map<String,Object> resultStrToken = JsonUtils.toObject(jsonResult, new TypeReference<Map<String,Object>>(){});
						...
						if (access_token == null || accessExpires == null) {
							...
							redisClient.set(redisAccessToken,access_token, Integer.valueOf(accessExpires.toString())-200);
						}
					} else {
				...
				Map<String,Object> result = JsonUtils.toObject(jsonResult, new TypeReference<Map<String,Object>>(){});
				...
				if(ticket!=null && ticketExpires!=null){
					...
					redisClient.set(redisJsapiTicket,ticket, Integer.valueOf(ticketExpires.toString())-200);
				}else{
